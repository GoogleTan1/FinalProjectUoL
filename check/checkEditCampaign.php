...
<head>
<meta charset="utf-8">
<title>Check Update Campaign</title>
</head>
<?php 
...
$campaignId = mysqli_real_escape_string($conn, $_POST["campaignId"]); //campaignName 
...
$displayOrg = mysqli_fetch_assoc($finalOrg);
 ...
 //-----------------------------IF NO IMAGE IS UPLOADED   	
    if ($uploadedFile == "")
    {
         $sqlInsertCampaign = "UPDATE campaigns SET campaignName = '$campaignName', campaignDetails = '$campaignDescription', finalAmount = '$finalAmount', bankAccountNo = '$bankAccountNo', bankType = '$bankAccountType' WHERE id = '$campaignId'";
         $insertCampaign = mysqli_query($conn,$sqlInsertCampaign); //Insert Campaign
         echo $sqlInsertCampaign;
         header("Location:../editCampaign.php?id=$campaignId&status=1");
  ...
  //-----------------------------IF THERE IS NO CAMPAIGN AT THE MOMENT     
        if ($campaignsFound == 0)  //No there is no campaigns at the moment
	    {
            ...
            if ($uploadOk == 0) 
            {
                header("Location:../editCampaign.php?id=$campaignId&status=2"); //File Format Wrong   
            } 
            else //uploads file
            {
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) //UPLOAD SUCCESS
                {
                    ...
                    $sqlInsertCampaign = "UPDATE campaigns SET campaignName = '$campaignName', campaignDetails = '$campaignDescription', finalAmount = '$finalAmount', bankAccountNo = '$bankAccountNo', campaignImage = '$fileNameChange' ,bankType = '$bankAccountType' WHERE id = '$campaignId'";
                    $insertCampaign = mysqli_query($conn,$sqlInsertCampaign); //Insert Campaign
                    echo $sqlInsertCampaign;
                    header("Location:../editCampaign.php?id=$campaignId&status=1");
                } 
                ...
                {
                    header("Location:../editCampaign.php?id=$campaignId&status=3"); //File not uploaded, error or above 1GB
                }
